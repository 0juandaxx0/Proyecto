...
# --- 2. CARGA DE DATOS Y ENTRENAMIENTO DE MODELO (CACHÉ) ---
@st.cache_data
def cargar_datos():
    # 1. Datos Universidad
...
# --- 3. ENCABEZADO ---
st.title("🚦 Observatorio de Movilidad y Rendimiento Académico")
st.markdown("Plataforma interactiva para la toma de decisiones basada en datos abiertos y encuestas universitarias en el corredor Bogotá-Soacha.")

# --- 4. CREACIÓN DE PESTAÑAS ---
tab1, tab2, tab3 = st.tabs(["🏙️ Contexto Ciudad (Movilidad)", "🎓 Impacto Estudiantil", "🤖 Simulador Predictivo AI"])

# ==========================================
# PESTAÑA 1: CONTEXTO CIUDAD
# ==========================================
with tab1:
    st.header("Análisis de la Infraestructura Urbana")
    
    # --- KPIs PRINCIPALES ---
    kpi_c1, kpi_c2, kpi_c3 = st.columns(3)
    total_pasajeros_dia = df_pasajeros['pasajeros_dia_tipico_laboral'].max()
    kpi_c1.metric("Volumen Diario (Bogotá/Soacha)", f"{total_pasajeros_dia:,.0f}".replace(',', '.'))
    ...
    with col_c1:
        # ==========================================
        st.subheader("Caída Histórica de la Velocidad")
        st.markdown("Evolución del tráfico vehicular en las últimas dos décadas sin duplicados.")
        ...
        # 3. SOLUCIÓN CRÍTICA: Eliminar duplicados de años, asegurar tipo entero y ordenar
        ...
        # 4. Crear el gráfico configurando de forma explícita el eje X como lineal continuo
    ...
    st.subheader("Presión del Sistema (Cuellos de Botella en Soacha)")
    st.markdown("Volumen de validaciones reales por estación y punto de acceso.")
...
with tab2:
    ...
    with col_e1:
        st.markdown("**Distribución por Medio de Transporte**")
...
# ==========================================
with tab3:
    st.header("🤖 Simulador Predictivo de Riesgo Académico")
    st.markdown("""
    ...
    with col_sim1:
        st.subheader("Parámetros del Estudiante")
        ...
        # Cálculo automático de horas semanales basado en los inputs
    ...
    with col_sim2:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            number = {'suffix': "%"},
            domain = {'x': [0, 1], 'y': [0, 1]},
            title = {'text': "Probabilidad de Riesgo Académico", 'font': {'size': 20}},
            gauge = {
        ...
        if probabilidad < 33:
            st.success("✅ Estudiante en zona segura. El impacto logístico es manejable.")
        elif probabilidad < 66:
            st.warning("⚠️ Riesgo moderado. El estudiante podría empezar a experimentar fatiga y fallas esporádicas.")
        else:
            st.error("🚨 Riesgo Crítico. Alta probabilidad de inasistencia recurrente o caída en el promedio. Se sugieren medidas de apoyo.")
